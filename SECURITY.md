...
Generated projects are authoring repositories. Hidden tests are not secret when teacher artifacts are distributed: reference implementations and verified tests are separated from the learner workspace, but remain inspectable by anyone with access to the complete repository. Use a private teacher repository or a learner-only export when secrecy is required.
