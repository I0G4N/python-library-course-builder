...
- [ ] Legacy input must own its Git top level. Stale plans, symlinks, containing roots, invalid result paths, transient rollback-path collisions, failed verification, candidate drift, either rename failure, and ordinary pre-cleanup validation failures leave or restore the old root byte-for-byte; externally occupied or changed recovery paths fail closed without deleting unverified trees.
- [ ] Successful apply leaves the live learner/author paths exactly equal to the verified candidate, durably records a committed/pending result before destructive cleanup, deletes the complete old Git/state/code/custom/build trees and the transient rollback directory, atomically upgrades the result to cleanup-complete, retains no backup, and reports the replacement as irreversible. A partial cleanup or final-result failure preserves the verified new pair and a truthful commit receipt instead of attempting an unsafe rollback.
