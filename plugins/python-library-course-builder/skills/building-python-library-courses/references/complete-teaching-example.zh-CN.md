...
这里的**接口**不是某个新类或新协议，而是两个组件之间可观察的输入、输出和失败边界。
调用者负责保留原始 JSON 文本并决定何时重试；解析边界负责把文本转换成 Python 值；object
检查负责只放行字典；字典消费者的职责只是按键读取已经通过检查的设置。这条依赖方向始终是
“消费者使用经过验证的字典”，消费者不反过来解释 JSON 记号。

下一章会把“解析 + 顶层 object 检查”集中到 `load_settings`。一个可信的备选方案，是在只有
一个调用点的一次性脚本里直接调用 `json.loads`，然后就地检查 `dict`。集中边界的好处是多个
调用者得到一致的输出与错误；代价是多出一个项目函数和一层跳转。它适用于多个消费者共享
同一顶层形态规则的项目；如果不同入口开始接受不同形态，或字段级校验成为主要职责，就应
重新选择接口边界，而不是继续扩大这个最小函数。
...
### 为什么把边界集中在 `load_settings`

`load_settings(text: str) -> dict[str, Any]` 是调用者与实现者之间的接口：调用者只承诺提供
JSON 文本，实现者则承诺返回通过顶层 object 检查的字典，或抛出前文已经观察过的异常。
组件间的数据流可以写成：

```text
caller -> load_settings: JSON 文本
load_settings -> json.loads: 同一文本
json.loads -> load_settings: 解析后的 Python 值
load_settings -> dict consumer: 通过 object 检查的字典
```

这个方向同时划清职责。caller 保留原始文本并决定恢复或重试；`json.loads` 只判断 JSON
语法；`load_settings` 负责本项目的顶层 object 规则；dict consumer 只读取 `enabled` 等键，
不重新解析文本。选择这一设计的备选方案，是让每个调用点各自执行 `json.loads` 和类型检查。
备选在单个短脚本中更直接，但调用点增多后容易产生不同的异常和放行规则。

集中边界的好处是依赖方向单一、契约可独立测试，且所有消费者共享恢复语义；代价是增加一层
间接调用，并且当前接口只保证“顶层是字典”，不保证字段存在或字段类型正确。它适用于当前
capstone 的共享 object 边界。若后续出现多种配置形态、字段默认值或版本迁移，就应重新选择
边界，考虑拆出 schema validator 或不同入口；这不会改变本章正在练习的 JSON-object 主线。
